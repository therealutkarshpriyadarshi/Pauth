...
try:
    ...
    raise ImportError(
        "Rich is required for OAuth Playground. "
...
class OAuthPlayground:
    ...
    def show_welcome(self):
        ...
        welcome_text = """
        [bold cyan]╔═══════════════════════════════════════════════════════╗[/]
        [bold cyan]║[/]  [bold magenta]🎨 PAuth OAuth 2.0 Interactive Playground[/]        [bold cyan]║[/]
        [bold cyan]╚═══════════════════════════════════════════════════════╝[/]
    ...
    def configure_client(self) -> OAuth2Client:
        ...
        client_secret = Prompt.ask("[cyan]Client Secret[/]", default="demo_secret", password=True)
        redirect_uri = Prompt.ask(
            "[cyan]Redirect URI[/]",
            default="http://localhost:8000/callback"
        )

        use_pkce = Confirm.ask("[cyan]Enable PKCE?[/]", default=True)

        scopes_input = Prompt.ask(
            "[cyan]Scopes (comma-separated)[/]",
    ...
    def show_token_details(self, tokens: TokenResponse):
        ...
        table.add_row(
            "Access Token",
            f"{tokens.access_token[:30]}..." if len(tokens.access_token) > 30 else tokens.access_token
        )
        ...
        if tokens.refresh_token:
            table.add_row(
                "Refresh Token",
                f"{tokens.refresh_token[:30]}..." if len(tokens.refresh_token) > 30 else tokens.refresh_token
            )
...
"""
    ...
    def test_flow(self, provider: Optional[str] = None):
        ...
        self.console.print("\n[bold cyan]═══ Step 1: Generate Authorization URL ═══[/]\n")
        ...
        state = Prompt.ask("[cyan]Enter the state parameter from callback[/]", default="")
        ...
        try:
            with Progress(
                ...
                self.tokens = self.client.exchange_code(code=code, state=state if state else None)
